...
.dataTables_wrapper .material-icons { 
    ...
    .link-produto {color: #0251ad; text-decoration: underline;}
</style>
...
<section class="panel">
        <header class="panel-heading">
            <h2 class="panel-title">Histórico de Resgate de Pontos</h2>
        </header>
...
function pesquisar()
{
    ...
    $.ajax({
        type: "POST",
        url: '/lojapontos/pesquisar-resgates-pontos/',
        cache: false,
        dataType: "json",
       cache: true,
        ...
        success: function(json){
            if (json.success)
            {
               ...
               table.clear().draw();
                ...
                {
                     table.row.add( [
                        json.message[i].dataTroca,
                        json.message[i].nomeUsuario,
                        json.message[i].nomeProduto,
                        json.message[i].pontosUtilizados,
                        json.message[i].quantidade,                   
                        '<a class="link-produto" href="/lojapontos/visualizar-produto/'+ json.message[i].idProduto +'" target="_BLANK" >'+ json.message[i].nomeProduto +'</a>',
